...
# iteration과 weight variable의 saver생성. 
saver = tf.train.Saver({iteration,W_conv1,W_conv2,b_conv1,b_conv2})
# 모든 variable에 대한 saver생성. 
saver2 = tf.train.Saver(tf.all_variables())

config = tf.ConfigProto(log_device_placement=False)
config.gpu_options.allow_growth = True #do not change
sess = tf.InteractiveSession(config=config)

sess.run(tf.initialize_all_variables())
#최근 파일의 variable을 불러옴
...
# 모든 variable 저장
saver2.save(sess, './log/model.ckpt', global_step=iteration)
