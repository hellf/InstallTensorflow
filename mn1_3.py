...
with tf.device('/gpu:0'):
  ...
  # L2 regularization loss 설정. 각 weight에 대한 l2 loss를 모두 더함.
  regularizers = (tf.nn.l2_loss(W_conv1) + tf.nn.l2_loss(W_conv2) +
                  tf.nn.l2_loss(W_fc1) + tf.nn.l2_loss(W_fc2))
  # L2 regularization loss와 cross entropy loss를 더하여 최종 loss를 설정.
  cross_entropy += 5e-4 * regularizers

  # learing rate: 0.01로 시작, image 5000장 마다 0.95만큼 exponential decay수행.
  learning_rate = tf.train.exponential_decay(
      0.01,        # Base learning rate.
      iteration * 16,  # Current index into the dataset.
      5000,       # Decay step.
      0.95,        # Decay rate.
      staircase=True)
  # 0.9의 momentum parameter 설정.
  train_step = tf.train.MomentumOptimizer(learning_rate,0.9,
    use_nesterov=False).minimize(cross_entropy,global_step=iteration)
